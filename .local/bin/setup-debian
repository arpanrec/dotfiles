...
export NEBULA_VERSION="${NEBULA_VERSION:-"1.14.61"}"
...
if [ "${CLOUD_INIT_INSTALL_DOTFILES}" = true ]; then
    bash <(curl -sSLf --connect-timeout 10 --max-time 10 \
        https://raw.githubusercontent.com/arpanrec/dotfiles/refs/heads/main/.local/bin/install-dotfiles)
fi

if [ "${CLOUD_INIT_IS_DEV_MACHINE}" = true ]; then
    bash <(curl -sSLf --connect-timeout 10 --max-time 10 \
        https://raw.githubusercontent.com/arpanrec/dotfiles/refs/heads/main/.local/bin/setup-workspace) \
        --tags all
fi
