...
APPIMAGE_NAME="bruno_${LATEST_VERSION#v}_${BRUNO_ARCH}_linux.AppImage"
...
echo "Downloading Bruno ${LATEST_VERSION} for ${BRUNO_ARCH} architecture"

if [[ ! -f "${TMP_DOWNLOAD_DIRECTORY}/${APPIMAGE_NAME}" ]]; then
	curl -fL --connect-timeout 10 --max-time 600 \
		"https://github.com/usebruno/bruno/releases/download/${LATEST_VERSION}/${APPIMAGE_NAME}" \
...
if [[ -f squashfs-root/usr/share/icons/hicolor/512x512/apps/bruno.png ]]; then
	mv \
		squashfs-root/usr/share/icons/hicolor/512x512/apps/bruno.png \
