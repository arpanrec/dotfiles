...
```bash
sudo -H -u root bash -c '/bin/bash <(curl -s https://raw.githubusercontent.com/arpanrec/dotfiles/main/workspace/bastille/debian/cloudinit-ansible.sh)'
```
...
```bash
bash <(curl https://raw.githubusercontent.com/arpanrec/dotfiles/main/workspace/bastille/server_workspace/webrun.sh)
```
